			...
			if ( isset( $wgLDAPPreferences[$_SESSION['wsDomain']] ) ) {
				...
				foreach ( array_keys( $prefs ) as $key ) {
					switch ( $key ) {
						...
						default :
							if ( isset( $info[0]["$prefs[$key]"] ) ) {
								$this->test[$key] = $info[0]["$prefs[$key]"][0];
								$this->printDebug( "Retrieved $key (".$this->test[$key].") using attribute ($prefs[$key])", NONSENSITIVE );
							}
					}
				}
 ...
 */
function AutoAuthSetup() {
	global $wgLDAPAutoAuthUsername, $wgLDAPSSLUsername, $wgLDAPAutoAuthDomain,
			$wgLDAPSmartcardDomain, $wgHooks, $wgAuth, $wgVersion;

	//HW
	// This is a nasty workaround to be able to log in local users. Hope
	// the extension itself will fix it asap :)
	if ( isset( $_REQUEST['wsDomain'] ) && 'local' == $_REQUEST['wsDomain'] ) {
		$_SESSION['wsDomain'] == 'local';
		return true;
	}
	//HW
