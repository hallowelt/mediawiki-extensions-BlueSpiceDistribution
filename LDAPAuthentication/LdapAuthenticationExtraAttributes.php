...
/**
 * Modification by Hallo Welt!
 *
 * - Additional Attributes are saved in the database. This requires an extra table. Creation code is below.
CREATE TABLE `hw_user_attributes` (
	`ua_user` INT(10) NULL DEFAULT NULL,
	`ua_attribute` VARCHAR(255) NULL DEFAULT NULL,
	`ua_value` VARCHAR(255) NULL DEFAULT NULL
)

 *
 * - Also, there are extra settings:
 * 	$hwgLDAPUseExtraAttributes = array('hallowelt' => true);
 *  $wgLDAPPreferences          = array( 'hallowelt' => array(  "email"=>"mail",
...
class LdapAuthenticationPlugin extends AuthPlugin {
			...
			if ( isset( $wgLDAPPreferences[$_SESSION['wsDomain']] ) ) {
				...
				foreach ( array_keys( $prefs ) as $key ) {
					switch ( $key ) {
						...
						case "language":
							if ( isset( $info[0]["$prefs[$key]"] ) ) {
								//$this->lang = $info[0][$prefs[$key]][0];
                                                                //HW
                                                                $this->lang = strtolower($info[0][$prefs[$key]][0]);
                                                                //HW
						...
						case "nickname":
							if ( isset( $info[0]["$prefs[$key]"] ) ) {
								$this->nickname = $info[0]["givenname"][0]." ".$info[0]["sn"][0]; //HW $info[0]["$prefs[$key]"][0];
								$this->printDebug( "Retrieved nickname ($this->nickname) using attribute ($prefs[$key])", NONSENSITIVE );
							}
							break;
						case "realname":
							if ( isset( $info[0]["$prefs[$key]"] ) ) {
								$this->realname = $info[0]["givenname"][0]." ".$info[0]["sn"][0]; //HW $info[0]["$prefs[$key]"][0];
						...
						default :
							if ( isset( $info[0]["$prefs[$key]"] ) ) {
								if (is_array($info[0]["$prefs[$key]"])) $this->additionalAttributes[$key] = $info[0]["$prefs[$key]"][0];
								else $this->additionalAttributes[$key] = $info[0]["$prefs[$key]"];
								$this->printDebug( "Retrieved $key (".$this->additionalAttributes[$key].") using attribute ($prefs[$key])", NONSENSITIVE );
							}
						// HW
					}
				}
	...
	function updateUser( &$user ) {
		...
		if ( ( isset( $wgLDAPRetrievePrefs[$_SESSION['wsDomain']] ) && $wgLDAPRetrievePrefs[$_SESSION['wsDomain']] )
			...
			// HW
			global $hwgLDAPUseExtraAttributes;
			if ( isset( $hwgLDAPUseExtraAttributes[$_SESSION['wsDomain']] ) )
			{
				$dbw = &wfGetDB( DB_MASTER );
				$dbw->delete('user_attributes', array('ua_user' => $user->getID()));
				foreach($this->additionalAttributes as $key=>$value)
				{
					$dbw->insert('user_attributes', array('ua_user' => $user->getID(), 'ua_attribute' => $key, 'ua_value' => $value)); //, array('ua_user' => $user->getID(), 'ua_attribute' => $key));
				}
			}
			// HW
		}
	...
	function getGroups( $ldapconn, $dn ) {
		...
		//$filter = "(&(sAMAccountName=$value)(objectclass=person))";
		...
		//HW 
		global $wgLDAPGroupUseMemberOf;
		if( $wgLDAPGroupUseMemberOf[$_SESSION['wsDomain']] )
		{
			//echo("xxxx "); print_r($entries[0]['memberof']);
			$groupentries = $entries[0]['memberof'];
			array_shift( $groupentries );
			foreach ( $groupentries as $entry ) {
				$mem = strtolower( $entry );
				preg_match('|^.*?=(.*?),.*$|', $mem, $matches);
				//print_r($matches);
				$shortnamemem = $matches[1];
...
function AutoAuthSetup() {
	...
	global $wgHooks;
	global $wgAuth, $wgVersion;

	//HW
        // This is a nasty workaround to be able to log in local users. Hope
        // the extension itself will fix it asap :)
	if ('local' == $_REQUEST['wsDomain'])
        {
            $_SESSION['wsDomain'] == 'local';
            return true;
        }
