 ...
 *  15.10.2010 topdate option added (TL.)
 */
...
function renderRss( $input ) {
	global $wgOutputEncoding, $wgParser, $wgLang;
	...
	if (isset($args["date"])) {
 	...
 	# Get topdate format from argument-array
	if (isset($args["topdate"])) {
		$topdate =  @$args["topdate"];
		if ($topdate == '')
			$topdate = 'd M Y';
	}
	else
	...
	if ( !$short and $description ) { #full item list
		...
		foreach ( $rss->items as $item ) {
			...
			// Add date as headline (TL. 15.10.1010)
			if ($topdate) {
				$toppubdate = trim( iconv( $charset, $wgOutputEncoding, $item['pubdate'] ) );
				if ( $toppubdate == '' ) {
					$toppubdate = trim( iconv( $charset, $wgOutputEncoding, $item['dc']['date'] ) );
				}
				$toppubdate = date( $topdate, strtotime( $toppubdate ) );
				$tpubunixsec = strtotime( $toppubdate );
				$tpubday = date( $topdate, $tpubunixsec );

				if ($thisdate != $tpubday) {
					$thisdate = $tpubday;
					$strdate = strftime( "%Y%m%d%H%M%S", $tpubunixsec );
					$formateddate = $wgLang->sprintfDate( $topdate, $strdate );
					$output.= "</ul><b>$formateddate</b><ul>";
				}
			}					
