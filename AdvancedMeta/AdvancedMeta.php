...
$wgExtensionMessagesFiles['AdvancedMeta'] = dirname( __FILE__ ) . '/AdvancedMeta.i18n.php';
...
class MWAdvancedMeta {
	...
	function onOutputPageBeforeHTML( &$out, &$text ) {
		...
		$out->addHeadItem( 'keywords', '<meta name="keywords" content="'.implode(',',$keywords).'" />' );
		...
		if ( isset( $meta['description'] ) ) {
			global $wgParser;
			if($wgParser instanceof Parser){}
			$out->addHeadItem( 'desc', '<meta name="description" content="'. $meta['description'].'" />' );
		}
